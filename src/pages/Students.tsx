...
export default function Students({
  ...
  const [bloodOptions, setBloodOptions] = useState<string[]>([]);
  ...
  const fetchStudents = async (
    ...
    try {
      ...
      // Accumulate filter options from all fetched records
      setCenterOptions((prev) =>
        unique([...prev, ...mapped.map((s) => s.trainingcenter)])
      );
      setBloodOptions((prev) =>
        unique([...prev, ...mapped.map((s) => s.blood_group)])
      );
      setQualOptions((prev) =>
        unique([...prev, ...mapped.map((s) => s.qualification)])
      );
    } catch (err: any) {
  ...
  // Reset page on dropdown filter / pageSize change
  useEffect(() => {
    setCurrentPage(1);
  }, [statusFilter, centerFilter, bloodFilter, qualFilter, pageSize]);

  // ── Client-side filtering (status + new fields) ────────────────
  const filtered = students.filter((s) => {
    if (statusFilter !== "All Status" &&
        s.status.toLowerCase() !== statusFilter.toLowerCase()) return false;
    if (centerFilter !== "All Centers" &&
        (s.trainingcenter ?? "").toLowerCase() !== centerFilter.toLowerCase()) return false;
    if (bloodFilter !== "All Blood Groups" &&
        (s.blood_group ?? "") !== bloodFilter) return false;
    if (qualFilter !== "All Qualifications" &&
        (s.qualification ?? "") !== qualFilter) return false;
    return true;
  });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.tornadoes.co.in/api/candidate/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="students-page">
      ...
      <main className="students-main">
        ...
        <div className="filters-card">
          ...
          {/* Training Center */}
          <div className="filter-group">
            <label className="filter-label">Training Center</label>
            <div className="select-wrap">
              <select
                className="filter-select"
                value={centerFilter}
              ...
              filtered.map((student) => (
                <div className="student-card" key={student.id}>
                  ...
                  <div className="student-left">
                    <div
                      className="student-avatar"
                      style={{ background: getColor(student.id ?? "0") }}
                    >
                    ...
                    <div className="student-info">
                      ...
                      <div className="student-meta">
                        ...
                        {student.blood_group && (
                          <span className="meta-chip meta-chip--blood">
                            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
                              <path d="M12 2C6.48 2 2 9 2 14a10 10 0 0 0 20 0c0-5-4.48-12-10-12z" />
                            </svg>
                            {student.blood_group}
                          </span>
                        )}
                        ...
                        {student.qualification && (
                          <span className="meta-chip meta-chip--qual">
                            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
